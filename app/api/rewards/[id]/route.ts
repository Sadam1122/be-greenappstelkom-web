...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const data = upsertRewardSchema.partial().parse(json)
    const updated = await prisma.reward.update({ where: { id: params.id }, data })
    return withCors(ok(updated, "Reward updated"), request)
  } catch (e) {
